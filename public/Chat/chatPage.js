...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function sendMessage(inputText) {
    try {
      if (!currentChatId) {
        // Create a new chat if no chat is currently selected
      ...
      // On success, replace the retry message with the response
      ...
      displayMessage(
        character,
        "", // No need for content since the message itself is "Try Again"
        true, // Indicates this is an error message
        (messageElement, retryMessage) => {
          // Retry the same message, and handle the retry button logic
